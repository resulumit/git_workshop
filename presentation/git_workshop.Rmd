...
---
## Command Line &mdash; `git` `branch` &mdash; Notes
...
15) Check the differences between the brances
- using the `git` `diff` command
    
`r countdown(minutes = 7, seconds = 30, top = 0)`

---
## Command Line &mdash; `git` `merge`

Use the `git` `merge` command to merge two branches
...
---
## Command Line &mdash; `git` `merge` &mdash; Notes

- Merge is done automatically if there are no conflicts
   - e.g., there are no edits on the same line of the same document in different branches
