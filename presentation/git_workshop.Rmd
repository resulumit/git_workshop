...
1) Move into the directory holding workshop materials
...
Use the `git` `init` command to turn a directory into a git repository
...
Initialized empty Git repository in ../pc/Desktop/resul_git_workshop/.git/
...
The `git` `init` command creates a .yellow-h[hidden] `.git` subdirectory

- will hold the versions of the files and folders tracked in our repository
    - nothing is being tracked yet
    - will be tracked as zipped, not meaningful to view at any stage
...
- The `git` `add` command is only the first of the two steps towards saving a version

   - there is no version of them saved yet
   - the tracked files are said to be .yellow-h[staged], or in the yewllow-h[staged area]
   
--

<br>

- The `git` `add` command can be reversed
...
- Use the `git` `commit` command to save a version
   - of the file or files that are already in the staged area
...
- This command requires a message
    - a message to yourself and to co-authors to indicate what has changed
...
- This command requires a message to be registered
    - a message to yourself and to co-authors to indicate what has changed
...
4) Save the current version of the file
   - using the `git` `commit` command
    
`r countdown(minutes = 3, seconds = 00, top = 0)`

---
## Command Line &mdash; `git` `add ` & `git` `commit` &mdash; Notes

- We use the combination of `git` `add ` & `git` `commit` commands very frequently

    - not only when we first start trackig a file
    - but also whenever we would like to create a new version

---
## Command Line &mdash; `git` `status`

Use the `git` `status` command to check the status of your repository
   - on which branch are you at?
   - are there any untracked, modified, deleted, staged ... files?
...
6) Make a change to the README.md file, save, and stage it
   - by double clicking on it in directory
       - which will opening it with the default text editor 
   - add and/or delete a sentence

7) Check the status of your repository

8) Commit a new version
   - recall the combination of commands
